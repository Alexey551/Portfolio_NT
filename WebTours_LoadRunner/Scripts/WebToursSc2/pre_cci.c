# 1 "c:\\loadtesting\\webtours\\scripts\\webtourssc2\\\\combined_WebToursSc2.c"
...
# 1 "c:\\loadtesting\\webtours\\scripts\\webtourssc2\\\\combined_WebToursSc2.c" 2
...
# 2 "c:\\loadtesting\\webtours\\scripts\\webtourssc2\\\\combined_WebToursSc2.c" 2
...
# 3 "c:\\loadtesting\\webtours\\scripts\\webtourssc2\\\\combined_WebToursSc2.c" 2
...
# 4 "c:\\loadtesting\\webtours\\scripts\\webtourssc2\\\\combined_WebToursSc2.c" 2
...
Login()
{
	...
	lr_end_transaction("00_OpenStartPage",2);
	...
	lr_end_transaction("01_DoLogin",2);
	
	lr_message(lr_eval_string("    {CURRTIME} : User is logged in with session {USERSESSION}"));
	
	return 0;
}
# 5 "c:\\loadtesting\\webtours\\scripts\\webtourssc2\\\\combined_WebToursSc2.c" 2
...
Action()
{
	...
	lr_end_transaction("02_OpenFlights",2);
	...
	if (strcmp(lr_eval_string("{CITIES_DEPART}"),lr_eval_string("{CITIES_ARRIVE}")) == 0){
		lr_message(lr_eval_string("           ERROR : Sity depart : {CITIES_DEPART} Sity arrive : {CITIES_ARRIVE}"));            	 
		while (strcmp(lr_eval_string("{CITIES_DEPART}"),lr_eval_string("{CITIES_ARRIVE}")) == 0){
			lr_save_string(lr_paramarr_random("CITIES"),"CITIES_ARRIVE");
		lr_message(lr_eval_string("           ERROR FIXED : Sity depart : {CITIES_DEPART} Sity arrive : {CITIES_ARRIVE}"));			 
		}
	}
	...
	web_submit_data("reservations.pl_2", 
		...
		"Name=depart", "Value={CITIES_DEPART}", "ENDITEM", 		 
		"Name=departDate", "Value={DEPARTDATE}", "ENDITEM",      
		"Name=arrive", "Value={CITIES_ARRIVE}", "ENDITEM", 		 
	...
	lr_end_transaction("03_DoSearchRoundTripTicket",2);
	...
	lr_end_transaction("04_SelectionRandomRoundTripFlights",2);
	
	lr_message(lr_eval_string("    {CURRTIME} : Found Seat Type : {SEATTYPE_RANDOM}, Seat Pref : {SEATPREF_RANDOM}"));
	
	return 0;
}
# 6 "c:\\loadtesting\\webtours\\scripts\\webtourssc2\\\\combined_WebToursSc2.c" 2
...
Logout()
{
	...
	lr_end_transaction("09_ DoLogout",2);
	
	lr_message(lr_eval_string("    {CURRTIME} : User is logged out of session"));

	return 0;
}
# 7 "c:\\loadtesting\\webtours\\scripts\\webtourssc2\\\\combined_WebToursSc2.c" 2
...
# 8 "c:\\loadtesting\\webtours\\scripts\\webtourssc2\\\\combined_WebToursSc2.c" 2
